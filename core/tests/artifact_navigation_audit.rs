...
#[test]
fn artifact_navigation_blocked_appends_events_receipt() {
    ...
    let blocked_url = "https://evil.example/steal";
    core.log_artifact_navigation_blocked(conversation.id.clone(), blocked_url.to_string())
        .expect("log");
    ...
    let blocked: Vec<_> = events
        .iter()
        .filter(|event| event["kind"] == "artifact_navigation_blocked")
        .collect();
