...
- Vault robustness rules: `meta.json` writes are atomic (temp + rename); reads are lock-free and tolerate a torn final `messages.jsonl` line in memory, while the write path repairs it on disk (interior corruption is a hard error); folder names are cosmetic and the id in `meta.json` is the truth (renames break nothing); duplicate ids from Finder copies or sync conflicts resolve deterministically to the newest `updated_at` and are reported via `issues()`; concurrency is any-readers/one-writer-per-conversation via `flock` on that conversation's `messages.jsonl` (no vault-wide lock; browsing while another instance runs is supported).
